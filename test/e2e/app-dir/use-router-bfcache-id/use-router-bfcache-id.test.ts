...
describe('use-router-bfcache-id', () => {
  ...
  it('preserves form state across search-param push', async () => {
    const { browser, act } = await setup('/x/1')
    await browser.elementByCss('[data-testid="leaf-input"]').type('hello')

    // Pushes router.push('?q=2').
    await act(async () => {
      await browser.elementByCss('[data-testid="push-search"]').click()
    })

    // Confirm the navigation took effect by waiting for the rendered
    // searchParams to reflect the new query.
  ...
  it('preserves form state across hash push', async () => {
    const { browser, act } = await setup('/x/1')
    await browser.elementByCss('[data-testid="leaf-input"]').type('hello')

    await act(async () => {
      await browser.elementByCss('[data-testid="push-hash"]').click()
    })
